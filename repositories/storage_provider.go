...
func (r *StorageProviderRepository) Create(ctx context.Context, storageProvider *storage_provider.StorageProvider) (*storage_provider.ID, error) {
	...
	protocolConnection, err := json.Marshal(storageProvider.ProtocolConnection)
	if err != nil {
		return nil, err
	}
	...
	_, err = r.db.ExecContext(ctx,
		...
		protocolConnection,
		// storageProvider.ProtocolConnection
	)
...
func (r *StorageProviderRepository) List(ctx context.Context) ([]*storage_provider.StorageProvider, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&storageProvider.ID, &storageProvider.Name, &storageProvider.FileSystem, &storageProvider.ProtocolConnection); err != nil {
			return nil, err
		}
